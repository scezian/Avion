    ...
    def __init__(self, parent=None):
        ...
        for b in (refresh_btn, choose_btn, cancel_btn):
            b.setStyleSheet(
                ...
                f"QPushButton:hover {{ background-color: {ACCENT_MUTED}; color: {ACCENT}; border-color: {ACCENT}; }}"
            )
...
class FullSpeedDurationDialog(QDialog):
    """Pick how long to run at full speed before auto-reverting to Auto mode."""

    PRESETS = [("1 min", 60), ("5 min", 300), ("10 min", 600), ("30 min", 1800), ("1 hour", 3600)]

    def __init__(self, parent=None):
        super().__init__(parent)
        self.seconds = None
        self.setWindowTitle("Full speed for how long?")
        ...
        head = QLabel("Auto-reverts to Auto mode after this long")
        head.setStyleSheet(f"color: {TEXT_DIM}; font-size: 12px;")
        ...
        for label, secs in self.PRESETS:
            ...
            btn.clicked.connect(lambda checked=False, s=secs: self._choose(s))
            layout.addWidget(btn)

        custom_row = QHBoxLayout()
        self.custom_input = QLineEdit()
        self.custom_input.setPlaceholderText("custom minutes")
        ...
        custom_btn.setStyleSheet(
            f"QPushButton {{ background-color: {BG}; color: {TEXT}; "
            f"border: 1px solid {BORDER}; border-radius: 6px; padding: 6px 14px; font-size: 13px; }}"
            f"QPushButton:hover {{ background-color: {ACCENT_MUTED}; color: {ACCENT}; border-color: {ACCENT}; }}"
        )
        custom_btn.clicked.connect(self._choose_custom)
        self.custom_input.returnPressed.connect(self._choose_custom)
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        full_btn.setStyleSheet(self._outline_btn_style())
        full_btn.setMinimumHeight(26)
        full_btn.clicked.connect(self.open_full_speed_picker)
        extra_row.addWidget(full_btn)
        control_card.layout_.addLayout(extra_row)

        self.full_speed_label = QLabel("")
        self.full_speed_label.setStyleSheet(f"color: {ACCENT}; font-size: 11px;")
        self.full_speed_label.setVisible(False)
        control_card.layout_.addWidget(self.full_speed_label)
        ...
        self.full_speed_timer = QTimer()
        self.full_speed_timer.setSingleShot(True)
        self.full_speed_timer.timeout.connect(self.set_auto)
        self.full_speed_countdown_timer = QTimer()
        self.full_speed_countdown_timer.timeout.connect(self._tick_full_speed_countdown)
        self.full_speed_remaining = 0
    ...
    def open_full_speed_picker(self):
        ...
        if dialog.exec() == QDialog.Accepted and dialog.seconds:
            self.start_timed_full_speed(dialog.seconds)

    def start_timed_full_speed(self, seconds):
        # write full-speed manually without triggering _cancel_full_speed_timer
        MODE_FILE.parent.mkdir(parents=True, exist_ok=True)
        LEVEL_FILE.write_text("full-speed")
        MODE_FILE.write_text("manual")
        self.auto_btn.setChecked(False)
        self.manual_btn.setChecked(True)
        self.level_row_wrap.setEnabled(True)
        ...
        self.full_speed_countdown_timer.start(1000)
        self.full_speed_label.setVisible(True)
        self._render_full_speed_countdown()

    def _tick_full_speed_countdown(self):
        self.full_speed_remaining -= 1
        if self.full_speed_remaining <= 0:
            self.full_speed_countdown_timer.stop()
            self.full_speed_label.setVisible(False)
        else:
            self._render_full_speed_countdown()

    def _render_full_speed_countdown(self):
        m, s = divmod(max(0, self.full_speed_remaining), 60)
        self.full_speed_label.setText(f"Full speed \u2014 reverting to Auto in {m}:{s:02d}")

    def _cancel_full_speed_timer(self):
